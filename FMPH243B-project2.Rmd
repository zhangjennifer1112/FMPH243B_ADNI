...
plot(logi.pred, logi.class.pred, col=test$cAD+10, pch=16)
...
plot(lasso.pred, lasso.pred.class, col=test$cAD+10, pch=16)
legend("right",legend = c("Non-converters", "Converters"),
       col=c(10,11), pch=16)
```
...
# logistic regression - completer: accuracy: 0.811 (95% CI: 0.7784, 0.8408)
logi.pred<- predict(fit.logi.back, newdata = basedf_1[,-1], type = "response")
logi.class.pred <- ifelse(logi.pred > 0.5, 1, 0)

(conf.mat <- confusionMatrix(as.factor(logi.class.pred),
                             as.factor(basedf_1$cAD)))
```
